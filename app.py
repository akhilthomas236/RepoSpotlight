...
# Import the database module via config manager
...
def ensure_db_initialized():
    ...
    try:
        # Check if the database initialization function exists and call it
        if hasattr(db, 'init_db'):
            db.init_db()
            
        # For SQLite databases, additional checks
        if hasattr(db, 'get_db_connection') and callable(db.get_db_connection):
            conn = db.get_db_connection()
            if conn is not None:  # Only proceed if we got a real connection (not None)
                cursor = conn.cursor()
        ...
        st.warning(f"Database check notice: {str(e)}")
    ...
    if repo_url:
        ...
        if db_repo:
            ...
            if repo_data:
                ...
                # Use the database version if GitHub API fails
                st.warning("Could not fetch fresh data from GitHub. Showing stored data.")
                
                # Display basic repository info from database
                st.header(db_repo["name"])
                st.subheader(f"by {db_repo['owner']}")
                st.write(db_repo["description"])
                
                # Stats
                col1, col2, col3 = st.columns(3)
                col1.metric("Stars", db_repo["stars"])
                col2.metric("Forks", db_repo["forks"])
                col3.metric("Language", db_repo["language"])
                
                # Metadata if available
                if db_repo["metadata"]:
                    st.header("Stored Metadata")
                    st.json(db_repo["metadata"])
        else:
...
def display_tile_view(repositories):
    ...
    if repositories:
        ...
        # Example showcase
        st.header("Welcome to RepoSpotlight")
        st.markdown("""
        RepoSpotlight allows you to showcase your best GitHub projects by:
        
        1. Displaying repository details and statistics
        2. Presenting README content in an attractive format
...
def main():
    ...
    with st.sidebar:
        ...
        <div style='text-align: center'>
            <h2>🚀 RepoSpotlight</h2>
            <p style='font-style: italic; font-size: 0.9em'>Shine a light on your best GitHub projects</p>
        </div>
        <hr>
        """, unsafe_allow_html=True)
        ...
        if st.checkbox("Show metadata template"):
            ...
            st.markdown("""
            ### How to use metadata in RepoSpotlight
            1. Create a `.github` folder in your repository
            2. Add a file named `project_metadata.json` with your custom metadata
            3. Copy the template above and customize it for your project
            4. RepoSpotlight will automatically use this metadata to enhance your project's display
            
            You can also use our [Metadata Generator](/Metadata_Generator) tool to create your metadata file easily!
            """)
